...
- **自动 / 手摇 双模式** — 底部分段按钮切换；手摇模式每爻都要一次独立手势（摇手机或点击铜钱），迟滞双阈值（`SHAKE_HI=18 / SHAKE_LO=6` m/s²）+ 爻间强制静候 300ms 防一次晃动串爆六爻；首爻额外 ~1100ms **书法金线描边仪式**（SVG `stroke-dashoffset` 从 12 点起笔绕外圆、补内方，三枚 stagger 150ms，`coin-fill-in` 末帧精确对齐 HOLD 稳态无跳帧）；每爻"握 → 摇 → 落"三段式视觉（摇动最少 600ms + 落地 ~560ms + 屏息 1000ms，系统端强制约 2.3s，叠加用户握持/起摇时长）；iOS 首次需授权 DeviceMotion，Android / iOS 旧版直接可摇，无传感器设备 2s 后自动提示点击 fallback
