...
- **自动 / 手摇 双模式** — 底部分段按钮切换；手摇模式每爻都要一次独立手势（摇手机或点击铜钱），迟滞双阈值（`SHAKE_HI=18 / SHAKE_LO=6` m/s²）+ 爻间静候 300ms（持续运动 ≥280ms 才催"请持稳手机"，防止"刚收手"的自然余动被误报）；**3 枚铜钱 DOM 整个起卦过程持续存在，不再每爻重建**（落定后留在原位，下一掷直接在原位继续抖动 —— 物理一致性强，无 DOM 重建闪烁）；**双模式首爻共用 ~1620ms 书法金线描边仪式**（SVG `stroke-dashoffset` 从 12 点起笔绕外圆、补内方，三枚 stagger 150ms；`coin-fill-in` 800ms 四段入场：overshoot 弹入 → 回落立定 → 保持锐利 ~100ms 可读「乾亨元利」字 → 缓溶入 HOLD 雾境 440ms；金线描边层 `sketch-dissolve` 同步淡出，不"线条独自残留"；末帧精确对齐 HOLD 稳态无跳帧）—— **手摇序章后切 `.holding` 等用户摇**（叙事："金笔生形" → "凝神静候"）；**自动序章后切 `.shaking` 接 600ms 自动翻转再落定**（首爻总头帧 ~2.2s，其余 5 爻保持纯翻转入场不带描边）；**大字三态语义**（`#tossInstr`）：off dim 灰白 = 等待 / **solid gold-hi** "金笔生形" = 描边期系统在画 / **gold 流光横扫** "凝神静候" = 现在可以摇（用户痛点修复："不知道什么时候可以摇"）；每爻"握 → 摇 → 落"三段式视觉（摇动最少 600ms + 落地 ~560ms + 屏息 1000ms，LAND→HOLD 用 600ms blur transition 平滑过渡，叠加用户握持/起摇时长）；iOS 首次需授权 DeviceMotion，Android / iOS 旧版直接可摇，无传感器设备 2s 后自动提示点击 fallback
